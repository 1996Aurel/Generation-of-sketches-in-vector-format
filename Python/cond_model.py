...
class Cond_Model():
    def __init__(self, hp, Nmax):
        if use_cuda:
            ...
            self.decoder = network.CondDecoderRNN(hp).cuda()
        else:
            self.encoder = network.EncoderRNN(hp)
            self.decoder = network.CondDecoderRNN(hp)
...
# The new function to train 
    def train(self, data, data_class, step): 
        ...
        # Now we add the condition to z : 
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        cond = (torch.ones([100, 1]) * data_class).to(device)
        z = (torch.cat((z, cond), axis = 1)).to(device)
    ...
    def conditional_generation(self,  data, data_class):   
        ...
        cond = (torch.ones([1, 1]) * data_class).to(device)
        z = (torch.cat((z, cond), axis = 1)).to(device)
        if use_cuda:
